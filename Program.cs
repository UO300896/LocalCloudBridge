...
// Load bridge configuration from appsettings.json
BridgeOptions options = BridgeOptions.Load(builder.Configuration);

// Configure Kestrel listening address
builder.WebHost.UseUrls(options.Listen);

// Allow large request bodies (unlimited length for streaming model payloads)
builder.Services.Configure<FormOptions>(o =>
{
    o.MultipartBodyLengthLimit = long.MaxValue;
});

// Configure HTTP client factory for proxying
builder.Services
    .AddHttpClient("proxy")
...
// Wait until remote target service becomes available (send Wake-on-LAN if offline)
